...
<script lang="ts" setup>
  ...
  const { t } = useI18n();
  const renderData = ref<ActionRecord[]>([]);
  ...
  const columns = computed<TableColumnData[]>(() => [
    ...
    {
      title: '额外数据',
      dataIndex: 'JSON.stringify("extra_data_for_event")',
    },
    {
      title: '批次ID',
      dataIndex: 'patch_id',
  ...
  const fetchData = async (
    params: ActionParams = { pageCurrent: 1, pageSize: 20 }
  ) => {
    setLoading(true);
    try {
      const data = await queryStatisticActions(params);
